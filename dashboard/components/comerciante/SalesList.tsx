...
function formatTime(sale: Sale): string {
  const dateValue = sale.timestamp || sale.created_at;
...
export default function SalesList({ sales, loading }: SalesListProps) {
  if (loading) {
    return (
        ...
        return (
          ...
          >
            <div className="flex items-center space-x-4">
              <div className={`w-11 h-11 rounded-xl flex items-center justify-center ${method === 'cash' ? 'bg-amber-50' : 'bg-blue-50'}`}>
                {method === 'cash'
              ...
              <div>
                <p className="text-lg font-bold text-gray-900 leading-tight">
                  S/ {parseFloat(String(sale.amount)).toFixed(2)}
                </p>
