...
interface RegisterFormProps {
  onRegister: (merchant: MerchantData) => void;
}

export default function RegisterForm({ onRegister }: RegisterFormProps) {
  const [formData, setFormData] = useState({ phone: '', businessName: '', location: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await registerMerchant(formData) as Record<string, unknown>;

      if (response.error) throw new Error(response.error as string);

      onRegister({
        id: response.id as string,
        merchantId: response.id as string,
        businessName: (response.name as string) || formData.businessName,
        location: (response.location as string) || formData.location,
        score: (response.score as number) || 75,
